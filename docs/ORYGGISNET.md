...
| **Payday-höfnun á rafrænum reikningi (XML) er aldrei þögul** (14.09.2026) | `payday-push` varaleiðin (`/electronic invoice/i`) kallar `skraXmlHofnun` bæði þegar endurtilraun án XML tekst og þegar hún mistekst; sendingin heldur áfram eins og áður, skráningin gleypir villur (3 s þak) og ber enga kt | `payday_xml_hafnad` (`warn` = „does not accept", annars `error`) | `audit-payday-xml-skraning.cjs` |
