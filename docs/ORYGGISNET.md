...
| **Payday-höfnun á rafrænum reikningi (XML) er aldrei þögul — og lendir hjá Agnari sem spurning** (14.09.2026, hert 15.09.) | `payday-push` varaleiðin (`/electronic invoice/i`) kallar `skraXmlHofnun` bæði þegar endurtilraun án XML tekst (á eftir `markSaleInvoiced`) og þegar hún mistekst; sendingin heldur áfram eins og áður. Skráningin fer í `app_problems` OG stofnar eitt mál per sölu á Þjónustuborðinu: á Agnar, merkt `samthykki` + `spurning` („Þarf svar frá þér" í 368aa), með samantekt „Þarf frá þér: …" eftir því hvort pósturinn fór, netfang vantaði, þetta voru drög eða enginn reikningur varð til. Hún gleypir villur (3 s þak á öllum þremur köllum) og ber enga kt í texta. `166` sýnir villuna strax í glugga (`synaXmlHofnun`, allur breytilegur texti um `esc`), bæði í stakri sendingu og fjöldasendingu | `payday_xml_hafnad` (`warn` = „does not accept", annars `error`) + mál með töggunum `payday-xml` + `spurning` | `audit-payday-xml-skraning.cjs` (keyrir `skraXmlHofnun` á gervineti og `synaXmlHofnun` í gervi-DOM) |
...
- **2026-09-23 (skuggahluturinn í `153` — VÖRÐUÐ skrá; „sumir vilja ekkert virkjast aftur eða óvirkjast")** — `_cache.byId` var byggt (`:397`) úr `allCompanies`, en `_cache.list` skilar **nýjum hlut per fyrirtæki** (`return { ...c, _ars, … }` í `loadAll`). Taflan teiknar úr `list`; hver einasti raðar-hnappur les `byId`. Þeir unnu því á sitt hvorum hlutnum. **Mælt á lifandi borði:** smellur á „↩ Virkja aftur" vistaði rétt á þjóninum (`ekki_sleppt` flakkaði true↔false við hvern smell, staðfest í `app_settings`) en merkið á röðinni breyttist ALDREI — líka þótt taflan væri rifin niður og byggð aftur (nýtt `tbody`, nýr hnappur) — og `cur` var lesið af skuggahlutnum, svo næsti smellur gat skrifað sama gildið aftur. `byId` vísar nú á teiknuðu hlutina (fyrirtæki utan borðsins halda sínum stað í kortinu). Ferðanótan slapp aðeins af því að hún skrifaði handvirkt í BÁÐA (`:2775–2776`) — sú lína var plástur á þessa rót. **Lærdómur:** hlutur sem tvær leiðir lesa verður að vera EINN hlutur; annars lýgur viðmótið þótt gögnin séu rétt. Engin breyting á reglum, talningu né skrifum. Sannreynt gegnum viðmótið fram og til baka, prófgögn skilin eftir óbreytt.
- **2026-09-23 (🟡 Slepptir = hrein bakstaða — `153`, sían sjálf)** — flagan taldi 132; **mælt:** 36 þeirra hafði Agnar sjálfur virkjað aftur (`ekki_sleppt`) og 5 eru afskráð félög (`fyrirtaekjaskra_stada`, sama heimild og ⛔-merkið í `380`). Hráa reglan frá 2026-08-17 (stjórnborðið) er nú á rofa: sjálfgefið sýnir sýnin það sem raunverulega er eftir að fara í (91), og „↩ sýna líka 41 undanskilda" í undirlínunni skilar hráu reglunni óbreyttri (`state.skipRaw`, staðbundið útlitsval). `countByStatus` keyrir sömu síu, svo flagan, súlan í mánaðastrimlinum og listinn geta ekki rekið í sundur. Afskráningin er AÐEINS lesin — engu breytt í gögnum, ekkert falið annars staðar, enginn tekinn úr þjónustu.
- **2026-09-23 (hopp við endurteikningu — `153` `render()`, AÐEINS skrunstaða)** — endurheimt í sama tifi (23.09, sjá að neðan) dugði ekki: **mælt** 2.200 → 2.959 px (759 px hopp) við EINA endurteikningu, innihaldshæð datt 4.705 → 3.967 px á meðan. Orsökin er ásynkrón — `187/267` sprauta árs-dálkum og akstur-merkjum inn EFTIR teikninguna. Skrunstaðan er nú sett aftur í þremur skrefum (rAF · 180 ms · 420 ms) og **hætt um leið og notandinn snertir skrunið sjálfur** (wheel/touch/keydown/mousedown), svo þetta togi aldrei á móti honum; `overflow-anchor:none` á `#ars-main`/`#view-arsskodun` (í `394`) slekkur á sjálfvirku akkeri vafrans. Mælt eftir á: **0 px** á 1.400 px og 412 px breidd, og notandi sem skrunar sjálfur heldur sínum stað.
- **2026-09-21 (afköst — 153 backgroundRefresh, VÖRÐUÐ skrá)** — endursókn alls mengisins (45 netköll) við hverja ferð til baka á Ársskoðun er nú SLEPPT í allt að 60 s ef ENGIN skrift fór úr flipanum síðan síðast (`RestSamnyting.skrif()` telur hverja skrift á Supabase — líka rpc/AppSettings — og á okkar eigin `/api/`-föll). Var eitthvað vistað → sótt strax eins og áður; sé lagið óvirkt gildir gamla 8 s reglan. Engin breyting á `loadAll`, „tilbúið"-rökum, talningu né skrifum. Afleiðing sem þarf að vita: breytingar ANNARRA véla sjást í versta falli 60 s seinna á lista sem þegar er opinn (áður 8 s).
- **2026-09-21 (gagnaverðir: 4 rauðir → 1)** — (1) `audit-payday-samraeming`: `T3:R-000418` → `T2:R-000418` í KVITTAD; frávikið skipti um flokk þegar Agnar hreinsaði rangt greiðslumerki 20.09, ákvörðun hans stendur í `krafa_note` („leyfa þessu að vera", endurrukkað sem R-000717). (2) `audit-kredit-tenging` ÞRENGDUR, grunnlína ÓBREYTT (2): kreditfærsla á walk-in móður (kt 999999-9999) fellir ekki — þar er ekkert fyrirtæki sem mínusinn gæti vantað á; talin og nafngreind í græna svarinu (R-000951 prufa, R-000959 Ármann). (3) `audit-t-s-i`: #228 Stjörnusól 3→2, #482 Ölfusborgir 42→39, #697 Grasnytjar 12→15 — `arsskodun_customers[fid].equipment` leiðrétt að því sem FJÓRAR heimildir sögðu samhljóða (uttaeki per tegund · arsskodun_report_facts 2026 · reikningslínur R-000966/977/976 · engin mótsögn); fyrri gildi geymd í `_leidrett_20260921` á hverri færslu. Ekkert rukkað eftir á. (4) `audit-solu-id` er ENN RAUÐUR af réttri ástæðu: R-000972 (10.540 kr) og R-000957 (1.426 kr) eru CO₂-drög „greitt síðar" á walk-in — mál #1076 og #1077 á borðinu (Samþykkja) bíða svars Agnars.
