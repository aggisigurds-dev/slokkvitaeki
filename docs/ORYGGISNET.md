...
| **Payday-höfnun á rafrænum reikningi (XML) er aldrei þögul** (14.09.2026) | `payday-push` varaleiðin (`/electronic invoice/i`) kallar `skraXmlHofnun` bæði þegar endurtilraun án XML tekst (á eftir `markSaleInvoiced`) og þegar hún mistekst; sendingin heldur áfram eins og áður. Skráningin fer í `app_problems` OG stofnar eitt mál per sölu á Þjónustuborðinu (á Agnar, `samthykki`), gleypir villur (3 s þak) og ber enga kt. `166` sýnir villuna strax í glugga (`synaXmlHofnun`) í stakri sendingu og fjöldasendingu | `payday_xml_hafnad` (`warn` = „does not accept", annars `error`) + mál með tagginu `payday-xml` | `audit-payday-xml-skraning.cjs` |
