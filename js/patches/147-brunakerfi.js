  ...
  function openEditDialog(coId) {
    ...
    dlg.querySelector('#_bk-edit-save').addEventListener('click', async () => {
      ...
      try {
        ...
        // Equipment links to the company by `client` name only, so a rename
        // would orphan every tæki until the name matched again. Cascade it.
        if (window.DB && DB.renameClientCascade && (co.nafn || '') !== nafn) {
          await DB.renameClientCascade(co.nafn, nafn);
        }
