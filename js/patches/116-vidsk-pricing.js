...
(() => {
  ...
  function renderSection(vid) {
    ...
    const rows = list.length ? list.map(p => {
      ...
      '</tr>';
    }).join('') : '<tr><td colspan="6" style="padding:18px;text-align:center;color:#94a3b8;font-size:13px;font-style:italic">Engin sérkjör skráð — smelltu „+ Ný sérkjör" hér að neðan</td></tr>';
    ...
    return '' +
      ...
      '</div>' +
      // Add new form
      '<div style="margin-top:10px;padding:12px;background:#fff;border:1px dashed #bfdbfe;border-radius:8px">' +
