...
(() => {
  ...
  function openDetail(coId) {
    ...
    bg.querySelector('._ars-info-save').addEventListener('click', async () => {
      ...
      // The name is the only link from tæki (uttaeki/lanstaeki) to the
      // company, so cascade a rename to keep them from being orphaned.
      if (window.DB && DB.renameClientCascade && patch.nafn && patch.nafn !== _oldNafn) {
        await DB.renameClientCascade(_oldNafn, patch.nafn);
      }
      bg.remove();
      render();
    });
