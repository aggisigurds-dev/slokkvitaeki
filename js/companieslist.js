...
function buildAllRows(){var v=document.getElementById('view-companies');var grid=v.querySelector('.company-grid');if(!grid)return [];var cards=grid.querySelectorAll('.company-card');var rows=[];Array.prototype.forEach.call(cards,function(card){var nameEl=card.querySelector('h2,h3,h4,strong,[class*="name"]');var name=nameEl?nameEl.textContent.trim():card.textContent.trim().split('\n')[0];var d=_cache && _cache[name] || {ext:0,hose:0,smoke:0,other:0,nextInsp:null};rows.push({name:name,card:card,data:d});});return rows;}
...
setInterval(function(){var v=document.getElementById('view-companies');if(!v||v.style.display==='none')return;var g=v.querySelector('.company-grid');if(g && !g.dataset._listShown)decorate();},1500);
setupObserver();
window.refreshCompaniesList=function(){_cache=null;var v=document.getElementById('view-companies');var g=v&&v.querySelector('.company-grid');if(g){g.dataset._listShown='';g.style.display='';var w=v.querySelector('._cl_wrap');if(w)w.remove();}decorate();};
